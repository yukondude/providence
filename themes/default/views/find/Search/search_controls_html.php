	...
	if (!$this->request->isAjax()) {
		...
		if (!$this->getVar('uses_hierarchy_browser')) {
			print caFormControlBox(
				...
				caFormSubmitButton($this->request, __CA_NAV_ICON_SEARCH__, _t("Search"), 'BasicSearchForm')
			);
		} else {
			print caFormControlBox(
				'<div class="simple-search-box">'._t('Search').': <input type="text" id="BasicSearchInput" name="search" value="'.htmlspecialchars($this->getVar('search'), ENT_QUOTES, 'UTF-8').'" size="40"/>'.$vs_query_builder_toggle.'</div>'.caFormSubmitButton($this->request, __CA_NAV_ICON_SEARCH__, _t("Search"), 'BasicSearchForm'),
				'<a href="#" onclick="caSaveSearch(\'BasicSearchForm\', jQuery(\'#BasicSearchInput\').val(), [\'search\']); return false;" class="button">'._t('Save search').'&nbsp;&rsaquo;</a>',
			...
			?>
				...
				<div id='browseTypeMenu'>
					<form action='#'>
						<div>
							<?php
							print _t('Add under %2 new %1', $this->getVar('type_menu').' <a href="#" onclick="_navigateToNewForm(jQuery(\'#hierTypeList\').val())">'.caNavIcon($this->request, __CA_NAV_ICON_ADD__, 1)."</a>", "<span id='browseCurrentSelection'></span>");
							?>
						</div>
					</form>
